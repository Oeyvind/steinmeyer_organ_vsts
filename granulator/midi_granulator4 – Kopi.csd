...
button  bounds( 55, 10, 55, 30), channel("all_off"), text("all_off"), latched(0), colour:0("black"), colour:1("red")
...
instr 1
  ...
  if trigger(kall_off, 0.5, 0) > 0 then
    ;turnoff2 197, 0, 1
    event "i", 197, 0, -1
  endif
...
instr 3,4,5,6,10
  ...
  k_no_fq_adjust = ichan == 1 ? 0 : 1 ; channel 1 is not allowed to adjust tempo
  ...
  kfqgain = chnget:k("sync_rate")*ksync_on*0.002*(1-k_is_master)*k_no_fq_adjust
  ...
  i_nstrnum = 201+(p1/100) 
  ;print i_nstrnum, i_nstrnum*100 
  if ktrig > 0 then
...
instr 197
  ; Sequential panic: one note-off style message per k-cycle
  kidx init 0
  ktotal = 128*5

  if kidx < ktotal then
    knote = kidx % 128
    kchan_idx = int(kidx/128)
    kchan = (kchan_idx == 4 ? 8 : kchan_idx+1)

    event "i", 198, 0, 0.01, knote, kchan

    kvoicetab table kchan-1, giVoices
    tablewkt 0, knote, kvoicetab
...
instr 198
  ; Single per-note note-off using Note On with velocity 0
  inote = p4
  ichan = p5

<Cabbage>
form caption("Register Sequencer") size(1220, 550), colour(30, 35, 40), guiMode("queue"), pluginId("rsq2")

nslider bounds(5, 45, 40, 20), channel("duration"), range(-1,1,0.5), fontSize(13)
...
instr 4
  ...
  ; Only trigger on note-on (vel > 0).
  if k_vel <= 0 then
    kgoto done
  endif
  
  ; Map MIDI channels to sequencer channels.
  ; Channel 1 (0-based: 0) -> seq ch1
  ; Channel 2 (0-based: 1) -> seq ch2
  ; Channel 3 (0-based: 2) -> seq ch3
  ; Channel 4 (0-based: 3) -> seq ch4
  ; Channel 8 (0-based: 7) -> seq ch8
  
  k_seq_chan = -1
  if k_chan == 1 then
    k_seq_chan = 1
  elseif k_chan == 2 then
    k_seq_chan = 2
  elseif k_chan == 3 then
    k_seq_chan = 3
  elseif k_chan == 4 then
    k_seq_chan = 4
  elseif k_chan == 8 then
    k_seq_chan = 8
  endif
  
  if k_seq_chan < 0 then
    kgoto done
  endif
  
  ; Get the modulo for this channel.
  k_numsteps = 8
  if k_seq_chan == 1 then
    k_numsteps chnget "numsteps"
  elseif k_seq_chan == 2 then
    k_numsteps chnget "ch2_numsteps"
  elseif k_seq_chan == 3 then
    k_numsteps chnget "ch3_numsteps"
  elseif k_seq_chan == 4 then
    k_numsteps chnget "ch4_numsteps"
  elseif k_seq_chan == 8 then
    k_numsteps chnget "ch8_numsteps"
  endif
  
  ; Calculate step using modulo.
  k_step = (k_note % k_numsteps) + 1
  
  ; Trigger the step playback.
  event "i", 60, 0, -1, k_step, k_seq_chan, kduration
  
  done:
endin

instr 60
  ; Step playback triggered by MIDI note.
  istep = p4
  iseq_chan = p5
  iduration = p6
  ...
  itab_group = giProg_tables_ch1
  Sout_chan_str = "outchan"
  Sndex_chan = "ndex"
  ioff = 0.0001
  
  if iseq_chan == 2 then
    itab_group = giProg_tables_ch2
    Sout_chan_str = "ch2_outchan"
      Sndex_chan = "ch2_ndex"
    ioff = 0.0003
  elseif iseq_chan == 3 then
    itab_group = giProg_tables_ch3
    Sout_chan_str = "ch3_outchan"
      Sndex_chan = "ch3_ndex"
    ioff = 0.0005
  elseif iseq_chan == 4 then
    itab_group = giProg_tables_ch4
    Sout_chan_str = "ch4_outchan"
      Sndex_chan = "ch4_ndex"
    ioff = 0.0007
  elseif iseq_chan == 8 then
    itab_group = giProg_tables_ch8
    Sout_chan_str = "ch8_outchan"
      Sndex_chan = "ch8_ndex"
    ioff = 0.0009
  endif
  
  iout_chan chnget Sout_chan_str
    ; Update step buttons to show which step is active.
    kbutn = 1
    while kbutn <= 8 do
      Sbutn_name sprintfk "%s_%i", Sndex_chan, kbutn
      if kbutn == istep then
        cabbageSetValue Sbutn_name, 1, 1
      else
        cabbageSetValue Sbutn_name, 0, 1
      endif
      kbutn += 1
    od
  ...
  ; Send program changes for all programs in this step.
  kndx = 0
  while kndx < 128 do
    kval table kndx, itab
    if kval > 0.5 then
      kinstrnum = 202 + ((kndx*0.001) + ioff)
      event "i", kinstrnum, 0, -1, kndx, iout_chan
    endif
    kndx += 1
  od
  
  ; Handle duration-based note off.
  if iduration >= 0 then
    ; Duration is positive: schedule note-off.
    k_note_duration init 0
    k_note_duration = iduration / (ksmps / sr)
    if k_note_duration > 0 then
      event "i", 61, k_note_duration, .1, istep, iseq_chan, iout_chan
    endif
  endif
endin

instr 61
  ; Delayed note-off for programs in a step.
  istep = p4
  iseq_chan = p5
  iout_chan = p6
  ...
  ; Turn off all programs in this step.
  ip = 0
  while ip < 128 do
    ival table ip, itab
    if ival > 0.5 then
      instr_num = 202 + ((ip*0.001) + ioff)
      event "i", -instr_num, 0, .1, ip, iout_chan
    endif
    ip += 1
  od
endin
...
instr 3
  ; REMOVED - Clock-based sequencer no longer used
endin

instr 12
  ; REMOVED - Playback state handler no longer used
endin

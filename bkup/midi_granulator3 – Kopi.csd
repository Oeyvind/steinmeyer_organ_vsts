<Cabbage>
form caption("Midi granulator 3") size(425, 305), colour(30, 35, 40), guiMode("queue"), pluginId("mgr3")
...
rslider bounds(365, 5, 50, 50), channel("transpose"), text("Transpose"), range(-60, 60, 0, 1, 1)
...
rslider bounds( 65, 65, 50, 50), channel("sync_rate"), text("s_Tpo"), range(0, 0.1, 0.001, 0.3, 0.0001)
...
rslider bounds(315, 65, 50, 50), channel("rtempo"), text("Rtempo"), range(0,1,0)
rslider bounds(365, 65, 50, 50), channel("master_tempo"), text("M_tmpo"), range(0.1,50,2)
...
opcode ClearButton, 0, i
  ...
  if trigger(kclear,0.5,0) > 0 then
    ...
    while kndx < 128 do
      ...
      kndx += 1
    od
    tablecopy iVoicetab, giVoiceclear
  endif
endop
...
instr 3,4,5,6,10
  ...
  itempo_init_offset chnget "t_init_offset"
  ...
  if ktrig > 0 && krtempo > 0 then
    ...
    if kfq > kin_tempo*(1+krtempo) then
      ktempo_update -= random(limit(kcount_up,kin_tempo*0.1,kin_tempo), kin_tempo*krtempo*1.8)
    elseif kfq < kin_tempo*(1-krtempo) then
      ktempo_update += random(limit(kcount_down,kin_tempo*0.1,kin_tempo), kin_tempo*krtempo*1.8)
    else  
      ktempo_update += random(-kin_tempo*krtempo*0.8,kin_tempo*krtempo*0.8)
    endif
    ktempo_update limit ktempo_update, 1, kin_tempo*2
  endif

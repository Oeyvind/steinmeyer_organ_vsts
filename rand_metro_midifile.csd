<Cabbage>
form caption("Rand/metro with midi file read") size(600, 670), colour(30, 35, 40), guiMode("queue"), pluginId("rmt2")
...
rslider bounds( 5,  60, 50, 50), channel("max_freq"), text("max_fq"), range(0.1,10,2, 0.3, 0.01)
rslider bounds(12,  67, 36, 36), channel("min_freq"), text("min"), range(0.1,10,0.5, 0.3, 0.01)
...
instr 1
  ; GUI control
  kplay chnget "Play"
  kplay = kplay > 0.5 ? 1 : 0 ; DAW automation might smooth the value and thus not reach zero
  ButtonEvent kplay, 2
endin
...
instr 2
  ...
  kcount init 0
  kmin_freq chnget "min_freq"
  kmax_freq chnget "max_freq"
  kmin_chg changed kmin_freq
  kmax_chg changed kmax_freq
  if kmin_chg > 0 && kmin_freq > kmax_freq then
    kmax_freq = kmin_freq
    cabbageSetValue "max_freq", kmax_freq, 1
  elseif kmax_chg > 0 && kmax_freq < kmin_freq then
    kmin_freq = kmax_freq
    cabbageSetValue "min_freq", kmin_freq, 1
  endif

  krand_tick init 0
  kcountdown init 0
  kcountdown = kcountdown - (ksmps / sr)
  if kcountdown <= 0 then
    krand_tick = 1
    kcountdown random (1 / kmax_freq), (1 / kmin_freq)
  else
    krand_tick = 0
  endif
  kforce_metro1 chnget "force_metro1"
  kforce_metro1 = kforce_metro1 > 0.5 ? 1 : 0 ; DAW automation might smooth the value and thus not reach zero
  kforce_metro2 chnget "force_metro2"
  kforce_metro2 = kforce_metro2 > 0.5 ? 1 : 0 ; DAW automation might smooth the value and thus not reach zero
  kforce_metro3 chnget "force_metro3"
  kforce_metro3 = kforce_metro3 > 0.5 ? 1 : 0 ; DAW automation might smooth the value and thus not reach zero
  ...
  korig_rhythm = korig_rhythm > 0.5 ? 1 : 0 ; DAW automation might smooth the value and thus not reach zero
  korig_clean chnget "orig_clean"
  korig_clean = korig_clean > 0.5 ? 1 : 0 ; DAW automation might smooth the value and thus not reach zero
  cabbageSetValue "midifile_monitor", korig_clean, changed(korig_clean)
  krand_midi chnget "rand_midi_switch"
  krand_midi = krand_midi > 0.5 ? 1 : 0 ; DAW automation might smooth the value and thus not reach zero
  kmidi_oct_shift chnget "midi_oct_shift"
  kmidi_oct_shift = kmidi_oct_shift > 0.5 ? 1 : 0 ; DAW automation might smooth the value and thus not reach zero
...
instr 4
  ...
  kmonitor = kmonitor > 0.5 ? 1 : 0 ; DAW automation might smooth the value and thus not reach zero
  ktest init 0 
  ktest += kTrig
  if kTrig == 1 then
